...
def analyse_trade_level(trade_events: pd.DataFrame,
    ...
    # ── Trace-level sampling (critical fix) ──────────────────────────────
    # Sample pipeline_run_ids (trade IDs), not individual event rows.
    #
    # Why this matters:
    #   Row sampling:   50,000 rows from 2.8M → some trades split across
    #                   sample boundary → TRADE_STARTED in sample but
    #                   TRADE_COMPLETED not sampled → appears as failed trade
    #                   → completeness = 49% (wrong, actual = 98%)
    #
    #   Trace sampling: 25,000 trade IDs → all events for each trade included
    #                   → completeness = 98% (correct)
    #                   → variant comparison valid (complete traces only)
    #
    # The grain field in the contract declares trade-level analysis.
    # Sampling must respect the declared grain.

    all_trade_ids   = trade_events['pipeline_run_id'].unique()
    n_sample_trades = min(sample_size, len(all_trade_ids))

    sampled_ids   = pd.Series(all_trade_ids).sample(
        n=n_sample_trades, random_state=RANDOM_SEED
    )
    fracture_cols = ['pipeline_run_id','activity','timestamp','team']
    trade_sample  = (
        trade_events[trade_events['pipeline_run_id'].isin(sampled_ids)]
        [fracture_cols]
        .reset_index(drop=True)
    )

    print(f"  Grain-level sampling:")
    print(f"    Total trades:   {len(all_trade_ids):,}")
    print(f"    Sampled trades: {n_sample_trades:,}  (trace sampling, not row sampling)")
    print(f"    Sample rows:    {len(trade_sample):,}")
