...
def part3_lifecycle():
    ...
    print()
    print("  v Correct: fracture deregister")
    print()
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("  v register → bootstrap → activate → run is always the order")
        print("  v DRAFT gate prevents circular measurement")
        print("  v --no-activate keeps DRAFT for human review")
        print("  v deregister deletes PNML (stale net = wrong scores)")
        print("  v deprecate keeps PNML (audit trail for dead pipelines)")
        print("  v RED preflight blocks conformance — fix logs first")
        print("  v AMBER preflight reduces confidence level")
        print("  v grain declares what pipeline_run_id means")
        print("  v chain passes upstream/downstream events with team tags")
        print("  v PELT changepoint runs automatically — check d.changepoint")
        print("═" * 65)
    finally:
        teardown()
